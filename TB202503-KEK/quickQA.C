...
const int jbnu_wform_factor = 1000;


void jbnu_daq_quickQA(int RunNo, int nEvtToRead, const char* inPath);
...
void quickQA(int RunNo = 2080, int nEvtToRead = 10000, const char* inPath = "data_hodoscope_raw"){
	...
	//jbnu_daq_quickQA(RunNo, nEvtToRead, inPath);
	bic_daq_quickQA(RunNo, nEvtToRead, inPath);
...
//--------------------------------------------------------------------------------------------------------
void bic_daq_quickQA(int RunNo = 2080, int nEvtToRead = 10000, const char* inPath = "data_hodoscope_raw")
{
	...
	const char* inFile = Form("%s/bic_daq_42_%i.dat", inPath, RunNo);
...
void jbnu_daq_quickQA(int RunNo = 2080, int nEvtToRead = 10000, const char* inPath = "data_hodoscope_raw")
{
	...
	TCanvas* c1 = new TCanvas("c1", Form("RUN %d, Hodoscope Channel QA", RunNo), -1, 0, 120*1.2*8*xCVS, 120*4*xCVS); c1->Divide(8, 4);
	...
	for (int a=0; a<nCh; a++)
	{
		...
		H1Temp->GetYaxis()->SetRangeUser(-1*jbnu_wform_factor, 10*jbnu_wform_factor);
	...
	while (data_read < file_size)
	{
		...
		*/
		...
		//cout << tcb_trigger_number << " " << tcb_trigger_time << " "  << channel << endl;
	...
	TCanvas* c2 = new TCanvas("c2", Form("RUN %d, JBNU DAQ Trigger QA", RunNo), -1, 150*4*xCVS+55, 300*1.2*2*xCVS, 300*xCVS); 
	c2->Divide(2,1);
	c2->cd(1);
